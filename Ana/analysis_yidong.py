...
dic_phone_day_regular=[] #通话天数的归一化值
#print len(dic_phone_day)
for item in dic_phone_day.values():
    dic_phone_day_regular.append(float(1.0*item/max(dic_phone_day.values())))
...
#计算属于不同类别的概率：
start_hour_weight=[] #记录通话时间段权重
use_time_weight=[] #通话时长权重
start_weekday_weight=[] #星期权重
init_type_weight=[] #呼叫类型权重
proba_parent=[] #存放是父母的可能性
proba_pair=[]  #存放是夫妻或情侣的可能性
for i in range(len(other_cell_phone)):
    #判断通话起始时间段
    if(start_hour[i]>=18 and start_hour[i]<=20 ):
        start_hour_weight.append(0.85)
    elif(start_hour[i]>=8 and start_hour[i]<=11 ):
        start_hour_weight.append(0.11)
    elif(start_hour[i]>=11 and start_hour[i]<=13 ):
        start_hour_weight.append(0.03)
    else:
        start_hour_weight.append(0.001)
    #判断通话时长：
    if(use_time[i]>300 and use_time[i]<=600):
        use_time_weight.append(0.25)
    elif(use_time[i]>600 and use_time[i]<=1800):
        use_time_weight.append(0.57)
    elif(use_time[i]>1800 and use_time[i]<=3600):
        use_time_weight.append(0.17)
    else:
        use_time_weight.append(0.01)
    #判断星期几：
    if(start_weekday>=1 and start_weekday<=4):
        start_weekday_weight.append(0.32)
    elif(start_weekday>=6 and start_weekday<=7):
        start_weekday_weight.append(0.64)
    else:
        start_weekday_weight.append(0.04)
    i+=1
#按照关系的键值从大到小进行排序
...
#print dic_name1 
#print dic
#print len(start_weekday)#1173
#print len(start_hour)#1173
#print len(init_type)#1173
#print len(use_time)#1173
print "通话记录数: ",len(other_cell_phone) #1173
print "号码数: ",len(dic_relation)#291
weekday={  #转换星期
    1:'星期一',
    2:'星期二',
    3:'星期三',
...
def data_to_csv():
    ...
    for item in start_date1:
        if(item in festival.keys()):
            ...
            festival_array.append('Na')
    #根据已有数组创建csv文件
    columns=['phone_num','weekday','start_hour','use_time','init_type','start_date','location','festival','other_phone_place','call_days','relation']
    dataframe=pd.DataFrame({'phone_num':other_cell_phone,'weekday':start_weekday,'start_hour':start_hour,'use_time':use_time,'init_type':init_type,'start_date':start_date1,'location':place,'festival':festival_array,'other_phone_place':phone_place,'call_days':phone_calldays,'relation':relation})
    dataframe.to_csv(address+'.csv',index=True,sep=',',columns=columns,encoding='utf-8') #默认写模式
#data_to_csv()
rawdata=pd.read_csv(address+'.csv') #MAC
...
def relationi_net():
    ...
    plot(g, **visual_style)
#call_timebyHour() #通话次数by时间段
#use_time_hour() #通话时长by时间段的散点图
#call_timebyDay() #通话次数by天
#use_time_fre() #通话时长和频率双线图
#use_timebyHour() #通话时长by时间段折线图
#plot_top10()  #Top10号码
#_Kmeans()  #Kmeans聚类
#relationi_net() #画出关系网
#plot_screen_num() #输出筛选号码
#plot_rela()
#print len(proba_parent)
...
def crime_form(address):
    ...
    list_2=[]
    list_3=[]
    list1=sorted(config['crime']["民警"].items(), key = lambda asd:asd[1], reverse=True)
    list2=sorted(config['crime']["时间"].items(), key = lambda asd:asd[1], reverse=True)
    list3=sorted(config['crime']["地点"].items(), key = lambda asd:asd[1], reverse=True)
    ...
    for i in range(index_start,index_end):
        s=s+str(a[i])
    per_info=config['crime']["个人信息"]['1']
    info=''
    if per_info in s:
        index=s.index(per_info)
        info=s[index+4:]
        m=pattern2.search(str(info)) #从匹配后面的第一个汉字作为个人信息的开始位置。
        index2=m.start(0)
        dic_ret['案情经过']=s[:index-1]
        dic_ret['个人信息']=info[index2:]
    else:
        dic_ret['案情经过']=s
        dic_ret['个人信息']='None'
    return dic_ret